...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            ...
            exchange_min_trade_krw=to_decimal(os.environ.get("EXCHANGE_MIN_TRADE_KRW", os.environ.get("COINONE_MIN_TRADE_KRW", "5000"))),
            ...
            markets_per_cycle=max(1, int(os.environ.get("MARKETS_PER_CYCLE", "20"))),
            ...
            decision_reward_horizon_minutes=max(30, int(os.environ.get("DECISION_REWARD_HORIZON_MINUTES", "360"))),
            env_file=os.environ.get("ENV_FILE", ".env"),
            sft_output_jsonl=os.environ.get("SFT_OUTPUT_JSONL", "./artifacts/coinone_sft_cases.jsonl"),
            ...
            ollama_gguf_adapter_path=os.environ.get("OLLAMA_GGUF_ADAPTER_PATH", "./artifacts/coinone-qwen3-lora.gguf"),
            ollama_deploy_base_model=os.environ.get("OLLAMA_DEPLOY_BASE_MODEL", "qwen3:4b-instruct-2507-q4_K_M"),
            ollama_deploy_model_name=os.environ.get("OLLAMA_DEPLOY_MODEL_NAME", "coinone-qwen3-sft"),
            ollama_modelfile_path=os.environ.get("OLLAMA_MODELFILE_PATH", "./artifacts/Modelfile.coinone-qwen3-sft"),
            ollama_update_env_after_deploy=asbool(os.environ.get("OLLAMA_UPDATE_ENV_AFTER_DEPLOY"), True),
            compare_input_jsonl=os.environ.get("COMPARE_INPUT_JSONL", os.environ.get("SFT_OUTPUT_JSONL", "./artifacts/coinone_sft_cases.jsonl")),
...
class CoinoneOnlyAIBot:
    ...
    def _lookup_completed_order(self, market: str, order_id: str) -> Optional[dict]:
        try:
            ...
            log(f"[COMPLETED ORDER LOOKUP WARN] coinone {market} order_id={order_id}: {exc}")
        return None
    ...
    def execute_limit_order_with_retry(self, market: str, side: str, price: Decimal, qty: Decimal, notional_krw: Decimal, reason: str) -> bool:
        ...
        for local_attempt in range(1, max_attempts + 1):
            ...
            if remaining_qty <= 0:
                ...
                return True
            if remaining_notional < self.cfg.exchange_min_trade_krw:
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Coinone AI bot + SFT/LoRA/Ollama workflow")
